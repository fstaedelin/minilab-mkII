    ...
    def Navigate(self, destination="children", logging=False):
        ...
        if destination == "parent":
            if len(self.current_path) > 0:  # Move to the parent context
                if not logging: 
                    self.Log("Moving to parent folder", "INFO", True)
            ...
            if destination not in self._get_current_context():
                if not logging: 
                    self.Log(f"Destination does not exist, creating nested context: {destination}", "INFO", True)
                self._set_new_context(destination)
            if not logging: 
                self.Debug(f"{destination}", True)
