...
import utility.fl_commands.playlist_manager as playlist_manager
...
class Actions:
    ...
    # ---------------------- TARNSPORT ACTIONS --------------------------
    @staticmethod
    def playpause(unused_param_value):
        transport.start()
    ...
    def mute_current_playlist_track(unused_param_value):
        """Mute playlist track"""
        playlist.muteTrack(playlist_manager.current_playlist_track())

    @staticmethod
    def playlist_track_next(unused_param_value):
        """Next playlist track"""
        next_track = min(playlist.trackCount(), playlist_manager.current_playlist_track() + 1)
        playlist_manager.set_playlist_track(next_track)

    @staticmethod
    def playlist_track_prev(unused_param_value):
        """Prev playlist track"""
        prev_track = max(1, playlist_manager.current_playlist_track() - 1)
        playlist_manager.set_playlist_track(prev_track)
